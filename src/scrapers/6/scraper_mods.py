...
def scrape_apr(context) -> list[dict]:
    ...
    # Step 1: visit home page to get cookies / bypass WAF
    _apr_warm_cookies(context)
    _sleep(1.5, 2.5)

    # Step 2: also visit the product category listing so the Referer chain looks natural
    category_url = "https://www.goapr.com/products/software/ecu_upgrade/"
    _fetch_page(
        context,
        category_url,
        wait_selector="main, .product-grid, a[href*='parts']",
        timeout=30_000,
    ...
    for url in APR_URLS:
        ...
        # Intercept the SPA's internal API calls while rendering the page
        soup, api_payloads = _fetch_page_with_network_capture(
            context,
            url,
            api_url_fragment="api.goapr.com",          # captured XHR base
            wait_selector="table, .performance-table, .product-performance, main, h1",
            timeout=40_000,
            referrer=category_url,
        )

        if not soup:
            # Hard 403 even with cookies — go straight to fallback
            table_rows = _apr_fallback_row(part, url)
            if table_rows:
                log.info(f"APR: page blocked, using fallback for {part}")
            else:
        ...
        # Attempt 1: captured API JSON (most reliable for a React SPA)
        table_rows = _apr_parse_json_api(api_payloads, url)

        # Attempt 2: structured HTML table
        if not table_rows:
            table_rows = _apr_parse_table(soup, url)

        # Attempt 3: raw text pattern matching
        if not table_rows:
            table_rows = _apr_parse_text_patterns(soup, url)

        # Attempt 4: curated fallback
        if not table_rows:
            table_rows = _apr_fallback_row(part, url)
            if table_rows:
...
def scrape_racechip(context) -> list[dict]:
    ...
    # RaceChip has several URL structures over the years — try all of them
    URL_PATTERNS = [
        "{base}/performance-chips/{make}/",
        "{base}/tuning/{make}/",
        "{base}/chip-tuning/{make}/",
    ]

    # API fragments to capture from XHR calls
    API_FRAGMENTS = ["api.racechip.com", "/api/", "/graphql", "vehicles", "products"]

    for make in RACECHIP_MAKES:
        make_rows = []

        for pattern in URL_PATTERNS:
            url = pattern.format(base=base, make=make)

            # Try each URL pattern with network interception
            soup = None
            api_payloads: list[dict] = []
            for fragment in API_FRAGMENTS:
                s, captured = _fetch_page_with_network_capture(
                    context,
                    url,
                ...
                if s:
                    ...
                    break  # got a valid page for this URL pattern
                _sleep(0.5, 1.0)
            ...
            # Priority 1: captured API JSON
            make_rows = _rc_parse_json_api(api_payloads, make, base)
            if make_rows:
                log.info(f"RaceChip {make.title()}: {len(make_rows)} from API JSON")
                break

            # Priority 2: inline vehicle cards in DOM
            make_rows = _rc_parse_cards(soup, make, base, url)
            if make_rows:
                log.info(f"RaceChip {make.title()}: {len(make_rows)} from DOM cards")
                break

            # Priority 3: follow model links on the page
            model_links = _rc_parse_model_links(soup, make, base, url)
            if model_links:
                log.info(
                    f"RaceChip {make.title()}: {len(model_links)} model links, following"
                )
                for model_url in model_links[:20]:
                    model_soup, model_payloads = _fetch_page_with_network_capture(
                        context,
                        model_url,
                        api_url_fragment="racechip.com",
                        wait_selector=(
                            "article, .VehicleCard, .performance-data, main"
                        ),
                        timeout=25_000,
                        referrer=url,
                    )
                    if model_soup:
                        make_rows.extend(
                            _rc_parse_json_api(model_payloads, make, base)
                            or _rc_parse_cards(model_soup, make, base, model_url)
                        )
                    _sleep(1.5, 3.0)
        ...
        if make_rows:
            ...
            log.warning(f"RaceChip {make.title()}: 0 live modifications (site blocked)")
