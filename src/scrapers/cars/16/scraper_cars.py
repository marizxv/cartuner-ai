...
def get_models(make_slug: str, make_url: str) -> list[tuple[str, str]]:
    """
    Return [(model_name, model_url)] for a make, using all available sources.

    URL structure on Parkers:
      /make/               — make homepage (shows only popular models)
      /make/specs/         — all-models index (lists EVERY model × generation)
                             Links on this page are listing-level hrefs:
                             /make/<model>/<gen-YEAR>/specs/
                             We extract the <model> slug from these to build
                             model URLs: /make/<model>/

    Discovery sources, in order:
      1. /make/specs/    → extract model slugs from 4-segment listing hrefs
      2. /make/          → catch any models only linked on the homepage
                           (both <a href> and data-href/data-url attributes)
    """
    ms  = re.escape(make_slug)

    # /make/<model>/                        — direct model href
    model_pat    = re.compile(rf"^/{ms}/([a-z0-9][a-z0-9-]{{1,60}})/$")
    # /make/<model>/specs/                  — model specs index (from /make/specs/)
    # /make/<model>/reviews/  etc.          — any 3-segment model sub-page
    # All of these tell us the model slug exists; we extract group(1).
    model_sub_pat = re.compile(rf"^/{ms}/([a-z0-9][a-z0-9-]{{1,60}})/[a-z0-9][a-z0-9-]{{0,40}}/$")
    # /make/<model>/<gen>/specs/            — listing href (4 segments)
    listing_pat  = re.compile(rf"^/{ms}/([a-z0-9][a-z0-9-]{{1,60}})/[^/]+/specs/$")
    # /make/<model>/specs/  or  /make/<model>/reviews/  etc. — 3-segment sub-pages
    # The second segment (sub) must NOT be a year or gen slug (those only have digits/dashes)
    # We use this to extract the model slug when the page links to model sub-pages.
    model_sub_pat = re.compile(rf"^/{ms}/([a-z0-9][a-z0-9-]{{1,60}})/[a-z][a-z0-9-]{{1,40}}/$")

    seen_slugs: set[str]              = set()
    models:     list[tuple[str, str]] = []

    def _add_slug(slug: str, name: str = "") -> None:
        """Register a model by its slug. Skips non-model slugs."""
        if slug in _NOT_A_MODEL or slug in seen_slugs:
            return
        seen_slugs.add(slug)
        model_url = f"{BASE}/{make_slug}/{slug}/"
        # Use slug-derived name — the raw text from the page is often noisy
        # (e.g. "Aygo X (26 onwards)View Specifications")
        label = slug.replace("-", " ").title()
        models.append((label, model_url))

    def _scan(soup: BeautifulSoup) -> None:
        """Extract model slugs from all link-bearing elements in soup."""
        candidates: list[tuple[str, str]] = []  # (href, visible_text)
        for a in soup.select("a[href]"):
            candidates.append((a.get("href", ""), a.get_text(strip=True)))
        for el in soup.find_all(attrs={"data-href": True}):
            candidates.append((el["data-href"], el.get_text(strip=True)))
        for el in soup.find_all(attrs={"data-url": True}):
            candidates.append((el["data-url"], el.get_text(strip=True)))

        for href, text in candidates:
            href = href.lower()
            m = listing_pat.match(href) or model_pat.match(href) or model_sub_pat.match(href)
            if m:
                _add_slug(m.group(1))

    def _scan_and_report(soup: BeautifulSoup, source_label: str) -> None:
        """_scan with INFO-level reporting of what was found and what was skipped."""
        ...
        for el in soup.find_all(attrs={"data-url": True}):
            candidates.append((el["data-url"], el.get_text(strip=True)))
        # Also scan every HTML attribute for make-slug paths — catches
        # href-like values on non-<a> elements (buttons, divs, etc.)
        attr_re = re.compile(rf"(/{re.escape(make_slug)}/[a-z0-9][a-z0-9/-]*/?)", re.I)
        for tag in soup.find_all(True):
            for attr_val in tag.attrs.values():
                if not isinstance(attr_val, str):
                    continue
                for m in attr_re.finditer(attr_val):
                    candidates.append((m.group(1), tag.get_text(strip=True)[:60]))
        # Scan inline <script> blocks for URL-like strings
        for script in soup.find_all("script"):
            txt = script.string or ""
            for m in attr_re.finditer(txt):
                candidates.append((m.group(1), ""))

        log.info(f"  [{source_label}] {len(candidates)} total href candidates on page")

        matched_listing, matched_model, skipped_blocklist, skipped_pattern = 0, 0, 0, 0
        for href, text in candidates:
            href_l = href.lower()
            m = (listing_pat.match(href_l)
                 or model_pat.match(href_l)
                 or model_sub_pat.match(href_l))
            if m:
                slug = m.group(1)
                if slug in _NOT_A_MODEL:
                    skipped_blocklist += 1
                else:
                    matched_model += 1
                    _add_slug(slug)
            else:
                if f"/{make_slug}/" in href_l and href_l.count("/") >= 3:
                    skipped_pattern += 1
                    if skipped_pattern <= 5:
                        log.debug(f"    NO MATCH: {href!r}")

        log.info(
            f"  [{source_label}] listing_matches={matched_listing} "
            f"model_matches={matched_model} "
            f"blocklist_skips={skipped_blocklist} "
            f"pattern_misses={skipped_pattern} "
            f"→ cumulative models={len(models)}"
        )

    # ── Source 1: /make/specs/ — the definitive all-models index ─────────
    specs_index_url = make_url.rstrip("/") + "/specs/"
    specs_soup = fetch(specs_index_url, referer=make_url)
    if specs_soup:
        _scan_and_report(specs_soup, f"{make_slug}/specs/")
    else:
        log.warning(f"  [{make_slug}/specs/] fetch failed — falling back to homepage only")

    # ── Source 2: /make/ homepage — catch anything /specs/ might miss ────
    make_soup = fetch(make_url, referer=BASE + "/cars/")
    if make_soup:
        before = len(models)
        _scan_and_report(make_soup, f"{make_slug}/")
        if len(models) > before:
            log.info(f"  +{len(models) - before} extra model(s) from homepage")

    log.info(f"  {make_slug}: {len(models)} model(s) found: "
             + ", ".join(slug for slug, _ in [(m[0], m[1]) for m in models]))
    return models
...
def get_variant_spec_urls(make_slug: str, model_slug: str, model_url: str) -> list[str]:
    ...
    # The trailing segment must not be a known non-gen keyword like "specs",
    # "review", "for-sale", etc. — those match the same 3-segment pattern but
    # are not generations.
    _NON_GEN_SEG = frozenset({
        "specs", "review", "reviews", "road-tests", "used-prices", "for-sale",
        "news", "advice", "compare", "leasing", "buying-guide", "owners-reviews",
        "owner-reviews", "nearly-new", "on-sale-soon", "insurance", "finance",
        "valuations", "electric", "hybrid", "used", "new", "shortlist",
    })
    ...
    def _enqueue_gen(href: str) -> None:
        ...
        if href in seen_gen:
            return
        # Reject known non-gen path segments (specs, review, for-sale, etc.)
        seg = href.rstrip("/").rsplit("/", 1)[-1]
        if seg in _NON_GEN_SEG:
            return
        seen_gen.add(href)
    ...
    def _collect_gen_links(soup: BeautifulSoup) -> None:
        """
        Scan a page for generation-level links and enqueue them.
        Handles both /gen/specs/ (listing_re) and /gen/ (gen_re) formats.
        Also scans data-href / data-url attributes for JS-driven pages.
        """
        candidates: list[str] = []
        for a in soup.select("a[href]"):
            candidates.append(a.get("href", ""))
        # Some Parkers pages use data-href or data-url on div/li cards instead of <a>
        for el in soup.find_all(attrs={"data-href": True}):
            candidates.append(el["data-href"])
        for el in soup.find_all(attrs={"data-url": True}):
    ...
    # ── Step 1: seed generation list from /make/model/specs/ ─────────────
    #
    # Parkers uses /make/model/specs/ as the "all generations" index page.
    # It lists every body style × year combination with links of the form
    # /make/model/<gen-YEAR>/specs/  — exactly our listing_re pattern.
    # The plain model page (/make/model/) only shows the newest generation,
    # so it is useless for discovering older ones.  We must start here.
    #
    specs_index_url  = model_url.rstrip("/") + "/specs/"
    specs_index_href = f"/{make_slug}/{model_slug}/specs/"

    specs_soup = fetch(specs_index_url, referer=model_url)
    if specs_soup:
        _collect_gen_links(specs_soup)
        log.debug(
            f"    {make_slug}/{model_slug}: {len(listing_queue)} generation(s) "
            f"found on /specs/ index"
        )

    # Fallback: also scan the plain model page in case /specs/ was empty or
    # returned 404 (some models don't have a /specs/ index page).
    if not listing_queue:
        model_soup = fetch(model_url, referer=f"{BASE}/{make_slug}/")
        if model_soup:
            _collect_gen_links(model_soup)

    # If /make/model/specs/ IS itself a listing page (single-gen model),
    # enqueue it directly so the BFS visits it.
    if not listing_queue:
        _enqueue_listing(specs_index_href)

    log.debug(
        f"    {make_slug}/{model_slug}: {len(listing_queue)} generation(s) seeded total"
    )
