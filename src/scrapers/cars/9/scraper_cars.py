...
def get_variant_spec_urls(make_slug: str, model_slug: str, model_url: str) -> list[str]:
    """
    Return all DATA-level variant spec URLs for a model, across all generations.

    Limits applied:
      GENS_PER_MODEL  — max number of generations to visit   (0 = all)
      VARIANTS_PER_GEN — max variants to keep per generation  (0 = all)

    Generation discovery strategy:
      1. Seed from model page: collect all listing (/gen/specs/) and gen (/gen/) hrefs.
      2. For each gen-page href found (without /specs/), fetch the gen page itself to
         find cross-generation navigation links — Parkers often only links to other
         generations from within a generation page, not from the model index.
      3. BFS over listing_queue, discovering more gens from each listing page visited.
      4. Both /gen/specs/ (listing) and /gen/ (gen-page) formats are tracked in
         separate seen-sets to avoid missed discoveries caused by format mismatch.
    """
    ms  = re.escape(make_slug)
    ...
    # GEN PAGE: /make/model/<gen-YEAR>/   (cross-gen link without /specs/)
    gen_re     = re.compile(rf"^/{ms}/{mds}/([^/]+)/$")
    # DATA:     /make/model/<gen-YEAR>/<variant>/specs/
    data_re    = re.compile(rf"^/{ms}/{mds}/([^/]+)/([^/]+)/specs/$")

    # Ordered list of listing hrefs (/gen/specs/) to visit
    listing_queue: list[str] = []
    seen_listing:  set[str]  = set()   # tracks /gen/specs/ hrefs
    seen_gen:      set[str]  = set()   # tracks /gen/ hrefs (without /specs/)
    all_data:      list[str] = []
    seen_data:     set[str]  = set()

    def _enqueue_listing(href: str) -> None:
        """Add a /gen/specs/ listing href to the queue if not already seen."""
        if href not in seen_listing:
            seen_listing.add(href)
            listing_queue.append(href)

    def _gen_href_to_listing(href: str) -> str:
        """Convert /make/model/gen-YEAR/ → /make/model/gen-YEAR/specs/ ."""
        return href.rstrip("/") + "/specs/"

    def _consider_gen_href(href: str) -> None:
        """
        Handle a /make/model/gen-YEAR/ href:
        - convert to listing format and enqueue
        - mark the gen href itself as seen to avoid redundant fetches
        """
        if href in seen_gen:
            return
        seen_gen.add(href)
        listing = _gen_href_to_listing(href)
    ...
    def _harvest_links(soup: BeautifulSoup) -> None:
        """
        Scan any page's <a href> tags and enqueue discovered generations
        and collect direct data links.
        """
        for a in soup.select("a[href]"):
            href = a.get("href", "")
            if data_re.match(href):
                _add_data(href)
            elif listing_re.match(href):
                _enqueue_listing(href)
            elif gen_re.match(href):
                _consider_gen_href(href)

    # ── Seed: collect listing hrefs from model page ───────────────────────
    model_soup = fetch(model_url, referer=f"{BASE}/{make_slug}/")
    if not model_soup:
        log.info(f"    {make_slug}/{model_slug}: 0 spec page(s) (model page failed)")
        return []

    _harvest_links(model_soup)

    # ── Extra seed: fetch each gen-page found on model page to discover
    #   cross-generation navigation links that Parkers only shows inside
    #   a generation page (not on the make/model index).
    #   We fetch these gen pages now (before the main BFS) so all generations
    #   are in the queue before we start scraping variants.
    #   We only fetch gen pages that were found on the model page itself;
    #   gen pages found during BFS are handled inline below.
    initial_gen_hrefs = list(seen_gen)   # snapshot of what model page gave us
    for gen_href in initial_gen_hrefs:
        gen_soup = fetch(BASE + gen_href, referer=model_url)
        if gen_soup:
            _harvest_links(gen_soup)

    log.debug(
        f"    {make_slug}/{model_slug}: {len(listing_queue)} generation(s) in queue "
        f"after seeding from model page"
    )

    # ── Process listing queue (BFS, respects GENS_PER_MODEL) ─────────────
    gens_visited = 0
    i = 0  # explicit index so the loop sees items appended during iteration

    while i < len(listing_queue):
        if GENS_PER_MODEL and gens_visited >= GENS_PER_MODEL:
            log.debug(f"    Reached GENS_PER_MODEL={GENS_PER_MODEL}, stopping.")
            break
        ...
        # Collect DATA links (variants) from this listing page
        data_hrefs: list[str] = []
        ...
        log.debug(
            f"      gen {listing_href}: {len(data_hrefs)} variant(s) found, "
            f"{len(_pick(data_hrefs))} kept"
        )

        # Discover more generations from cross-gen links on this listing page.
        # Use _harvest_links so both /gen/specs/ and /gen/ formats are handled.
        _harvest_links(listing_soup)

        # For any newly discovered gen-page hrefs, fetch the gen page too
        # so we don't miss generations that are only linked from gen pages.
        for gen_href in list(seen_gen):
            # _consider_gen_href already marked them; fetch ones not yet fetched.
            # We track "fetched gen pages" via seen_gen — already populated by
            # _consider_gen_href, so just fetch pages added since last iteration.
            pass  # gen pages from listing pages are handled via listing_re links

        # Fallback: no data links → try one sub-level
        if not data_hrefs:
