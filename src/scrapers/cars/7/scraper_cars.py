...
def get_variant_spec_urls(make_slug: str, model_slug: str, model_url: str) -> list[str]:
    """
    Return DATA-level spec URLs for ALL generations of this model:
        /make/model/gen-YEAR/variant/specs/

    Walk:
        model page
          → listing pages  (/make/model/gen-YEAR/specs/)   ← one per generation
              → cross-gen listing links found on listing pages (older generations!)
              → variant DATA pages (/make/model/gen-YEAR/variant/specs/)

    Variant cap: VARIANTS_PER_GEN controls how many variants are kept per
    generation listing.  0 means unlimited (--all-variants mode).
    Variants are kept in page order so the first N tend to be the most
    representative trims (base, mid, top — as Parkers orders them).
    """
    ...
    # All DATA urls returned at the end
    all_data:     list[str] = []
    seen_data:    set[str]  = set()
    seen_listing: set[str]  = set()

    def _pick_variants(hrefs: list[str]) -> list[str]:
        """Apply VARIANTS_PER_GEN cap (0 = keep all)."""
        if VARIANTS_PER_GEN == 0:
            return hrefs
        return hrefs[:VARIANTS_PER_GEN]

    def _add_data_batch(hrefs: list[str]) -> None:
        for h in _pick_variants(hrefs):
            full = BASE + h
            if full not in seen_data:
                seen_data.add(full)
                all_data.append(full)

    def _process_listing(listing_href: str, referer: str) -> None:
        """
        Visit one listing page, collect:
          - variant DATA links  (apply cap per listing)
          - any cross-generation listing links (other gen-YEARs for same model)
        """
        if listing_href in seen_listing:
            return
        seen_listing.add(listing_href)

        listing_url  = BASE + listing_href
        listing_soup = fetch(listing_url, referer=referer)
        if not listing_soup:
            return

        # Collect variant DATA links and cross-gen listing links
        data_hrefs: list[str] = []
        new_listing_hrefs: list[str] = []

        for a in listing_soup.select("a[href]"):
            href = a.get("href", "")
            if data_re.match(href):
                if BASE + href not in seen_data:
                    data_hrefs.append(href)
            elif listing_re.match(href) and href not in seen_listing:
                # Cross-link to another generation (e.g. "See 2012 version")
                new_listing_hrefs.append(href)

        _add_data_batch(data_hrefs)
        log.debug(
            f"      {listing_href} → {len(data_hrefs)} variant(s) found, "
            f"{min(len(data_hrefs), VARIANTS_PER_GEN) if VARIANTS_PER_GEN else len(data_hrefs)} kept"
        )

        # Recurse into cross-gen listings (discovers older generations)
        for cross_href in new_listing_hrefs:
            _process_listing(cross_href, listing_url)

        # Fallback: if no DATA links, try one sub-level deeper
        if not data_hrefs:
    ...
    # Collect initial listing hrefs from model page
    initial_listings: list[str] = []
    for a in model_soup.select("a[href]"):
        href = a.get("href", "")
        if listing_re.match(href) and href not in seen_listing:
            initial_listings.append(href)
        elif data_re.match(href):   # direct data link (rare)
            if BASE + href not in seen_data:
                seen_data.add(BASE + href)
                all_data.append(BASE + href)

    # Process all listing pages (each may discover more via cross-gen links)
    for listing_href in initial_listings:
        _process_listing(listing_href, model_url)

    total_gens = len(seen_listing)
    log.info(
        f"    {make_slug}/{model_slug}: {len(all_data)} variant spec page(s) "
        f"across {total_gens} generation(s)"
    )
    return all_data
