...
def get_variant_spec_urls(make_slug: str, model_slug: str, model_url: str) -> list[str]:
    ...
    # Ordered list of listing hrefs to visit (newest first)
    ...
    def _enqueue_listing(href: str) -> None:
        """Add a listing href to the queue if not already seen."""
    ...
    for a in model_soup.select("a[href]"):
        ...
        if listing_re.match(href):
            ...
            # gen page without /specs/ — convert and enqueue
    ...
    for listing_href in listing_queue:           # list grows as we discover more
        ...
        for a in listing_soup.select("a[href]"):
            ...
            if listing_re.match(href):
                _enqueue_listing(href)
            elif gen_re.match(href) and href not in seen_listing:
                # /make/model/gen-YEAR/ link — add as listing
                listing = _gen_href_to_listing(href)
