...
def get_models(make_slug: str, make_url: str) -> list[tuple[str, str]]:
    ...
    # Pattern for a direct model href: /make/<model>/
    model_pat   = re.compile(rf"^/{ms}/([a-z0-9][a-z0-9-]{{1,60}})/?$")
    # Pattern for a listing href: /make/<model>/<gen>/specs/
    # We extract the model slug from group(1).
    ...
    def _scan(soup: BeautifulSoup) -> None:
        ...
        for href, text in candidates:
            href = href.lower()
            # Try listing href first (most specific): /make/model/gen/specs/
            m = listing_pat.match(href)
            if m:
                _add_slug(m.group(1), text)
                continue
            # Fall back to direct model href: /make/model/
            m = model_pat.match(href)
            if m:
                _add_slug(m.group(1), text)
        ...
        for href, text in candidates:
            ...
            if m:
                ...
                if slug in _NOT_A_MODEL:
                    skipped_blocklist += 1
                    log.debug(f"    SKIP (blocklist) listing slug={slug!r} href={href!r}")
                else:
                    matched_listing += 1
                    _add_slug(slug, text)
                continue
            m = model_pat.match(href_l)
            if m:
                slug = m.group(1)
                if slug in _NOT_A_MODEL:
                    skipped_blocklist += 1
                ...
                # Log hrefs that contain the make slug but don't match either pattern
                if f"/{make_slug}/" in href_l and href_l.count("/") >= 3:
                    skipped_pattern += 1
                    if skipped_pattern <= 30:
                        log.info(f"    NO MATCH: {href!r}")
