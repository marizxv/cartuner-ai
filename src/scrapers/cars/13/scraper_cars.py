...
def get_models(make_slug: str, make_url: str) -> list[tuple[str, str]]:
    ...
    # This page lists every model × generation combination, so it reliably
    # exposes models that never appear on the make homepage (e.g. GT86, Supra).
    specs_index_url = make_url.rstrip("/") + "/specs/"
    specs_soup = fetch(specs_index_url, referer=make_url)
    if specs_soup:
        _scan(specs_soup)
        log.debug(f"  {make_slug}: {len(models)} model(s) from /specs/ index")
    ...
    if make_soup:
        ...
        if len(models) > before:
            log.debug(f"  {make_slug}: +{len(models) - before} model(s) from homepage")
