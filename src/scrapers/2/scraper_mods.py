...
Джерела:
  1. APR (goapr.com)         — ECU тюнінг VAG
  2. RaceChip (racechip.com) — ECU тюнінг всіх марок
  3. Розширена статична база — 40+ модифікацій з реальними числами

Встановити: pip install cloudscraper lxml beautifulsoup4 pandas
Запуск:
    python scraper_mods.py               # всі джерела
    python scraper_mods.py --static-only # тільки статика (без інтернету)
"""
...
def scrape_apr() -> list[dict]:
    ...
    for url in APR_URLS:
        ...
        # Спроба 2: текстові патерни якщо таблиця не знайдена
        text = soup.get_text(" ")
...
# ─────────────────────────────────────────────────────────────────────────────
# ДЖЕРЕЛО 2: RaceChip (racechip.com) — ECU для всіх марок
# ─────────────────────────────────────────────────────────────────────────────
RACECHIP_MAKES = [
...
def scrape_racechip() -> list[dict]:
    ...
    for make in RACECHIP_MAKES:
        ...
        # RaceChip показує картки моделей з hp до/після
        for card in soup.select(".model-card, .car-card, .product-item, article"):
            ...
            # Шукаємо "from Xhp to Yhp" або "+Z hp"
...
# ─────────────────────────────────────────────────────────────────────────────
# СТАТИЧНА БАЗА — 40 модифікацій з реальними типовими числами
# Джерела: carthrottle.com, r/cars, r/projectcar, eEuroparts, fcpeuro
# ─────────────────────────────────────────────────────────────────────────────
STATIC_MODS = [
    # ── ECU ТЮНІНГ ──────────────────────────────────────────────────────────
